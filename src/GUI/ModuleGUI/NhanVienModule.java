...
import librarymanagementsystem.GUI.ThemSuaGUI.ThemNhanVienForm;
...
public class NhanVienModule {
    ...
    private void initComponents() {
        ...
        checkbox = new ComboCheckBox(new ArrayList<String>(Arrays.asList(nhanVienBUS.getHeaders())));
...
//        cb.setLocation(150, 115);
    ...
    private void nhapexcel_btnMouseClicked(java.awt.event.MouseEvent evt) {
        ...
        boolean finish = true;
        for (QLNhanVienDTO e : sach){
            if (!nhanVienBUS.add(e)){
                finish = false;
    ...
    private void searchtextfieldKeyReleased(java.awt.event.KeyEvent evt) {
        ...
        //ArrayList <QLNhanVienDTO> loaiSach = nhanVienBUS.search_all(search_str);

        /** Testing **/
        ArrayList <String> columns_checked = checkbox.getChecked();

        ArrayList <QLNhanVienDTO> nhanVien = nhanVienBUS.getArrNhanVien();
        ArrayList <QLNhanVienDTO> search_res = new ArrayList<>();
        ArrayList <QLNhanVienDTO> search_temp = new ArrayList<>();
        for (String e : columns_checked){
            search_temp = nhanVienBUS.search(e, search_str);

            ArrayList <String> pkey_1 = new ArrayList<>();
            ArrayList <String> pKey_2 = new ArrayList<>();

            for (QLNhanVienDTO ele : search_res){
                pkey_1.add(ele.getMaNhanVien());
            }

            for (QLNhanVienDTO ele : search_temp){
                pKey_2.add(ele.getMaNhanVien());
            }

            search_res = nhanVienBUS.getNhanVien_full(new DataProcessing().union_arr(pkey_1, pKey_2));
        }
        ...
        /* Print Ma Sach
        for (QLNhanVienDTO e : loaiSach){
            System.out.println(e.getMaSach());
        }*/

        jPanel1.remove(nhanVien_Table);
        jPanel1.repaint();
        jPanel1.revalidate();
        nhanVien_Table = new QLNhanVienTable().getTable(nhanVien);
        nhanVien_Table.setBounds(42, 150, 860, 440);
        jPanel1.add(nhanVien_Table);
    }

    public void paintTable(ArrayList <QLNhanVienDTO> nhanVien){
        jPanel1.remove(nhanVien_Table);
        jPanel1.repaint();
        jPanel1.revalidate();
        nhanVien_Table = new QLNhanVienTable().getTable(nhanVien);
    ...
    private static JPanel nhanVien_Table;
