...
public class PhieuNhapModule {
    private static QLPhieuNhapBUS phieuNhapBUS = new QLPhieuNhapBUS();
    boolean tooglesearch = false;
    
    public JPanel getPhieuNhapModule() {
        initComponents();
    ...
    private void initComponents() {
        ...
        checkbox = new ComboCheckBox(new ArrayList<String>(Arrays.asList(phieuNhapBUS.getHeaders())));
...
//        cb.setLocation(150, 115);
        ...
        phieuNhap_Table = new QLPhieuNhapTable().getTable();
        phieuNhap_Table.setBounds(42, 150, 860, 440);
        jPanel1.add(phieuNhap_Table);
...
//        }
    ...
    private void xuatexcel_btnMouseClicked(java.awt.event.MouseEvent evt) {
        ...
        new ExportFile().writeFileExcel_QLPhieuNhap();
    }
    ...
    private void searchtextfieldKeyReleased(java.awt.event.KeyEvent evt) {
        ...
        //ArrayList <QLPhieuNhapDTO> loaiSach = phieuNhapBUS.search_all(search_str);

        /** Testing **/
        ArrayList <String> columns_checked = checkbox.getChecked();

        ArrayList <QLPhieuNhapDTO> phieuNhap = phieuNhapBUS.getArrNhapKho();
        ArrayList <QLPhieuNhapDTO> search_res = new ArrayList<>();
        ArrayList <QLPhieuNhapDTO> search_temp = new ArrayList<>();
        for (String e : columns_checked){
            search_temp = phieuNhapBUS.search(e, search_str);

            ArrayList <String> pkey_1 = new ArrayList<>();
            ArrayList <String> pKey_2 = new ArrayList<>();

            for (QLPhieuNhapDTO ele : search_res){
                pkey_1.add(ele.getMaNhap());
            }

            for (QLPhieuNhapDTO ele : search_temp){
                pKey_2.add(ele.getMaNhap());
            }

            search_res = phieuNhapBUS.getPhieuNhap_full(new DataProcessing().union_arr(pkey_1, pKey_2));
        }

        phieuNhap = search_res;
        /** End Testing **/

        /* Print Ma Sach
        for (QLPhieuNhapDTO e : loaiSach){
            System.out.println(e.getMaSach());
        }*/

        jPanel1.remove(phieuNhap_Table);
        jPanel1.repaint();
        jPanel1.revalidate();
        phieuNhap_Table = new QLPhieuNhapTable().getTable(phieuNhap);
        phieuNhap_Table.setBounds(42, 150, 860, 440);
        jPanel1.add(phieuNhap_Table);
    }

    public void paintTable(ArrayList <QLPhieuNhapDTO> phieuNhap){
        jPanel1.remove(phieuNhap_Table);
        jPanel1.repaint();
        jPanel1.revalidate();
        phieuNhap_Table = new QLPhieuNhapTable().getTable(phieuNhap);
    ...
    private void expand_btnMouseClicked(java.awt.event.MouseEvent evt){
        new QLPhieuNhapTable().expandMode();
    }
    ...
    private static JPanel phieuNhap_Table;
