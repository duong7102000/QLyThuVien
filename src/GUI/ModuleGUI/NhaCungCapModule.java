...
public class NhaCungCapModule {
    private static QLNhaCungCapBUS nhaCungCapBUS = new QLNhaCungCapBUS();
    boolean tooglesearch = false;
    
    public JPanel getNhaCungCapModule() {
        initComponents();
    ...
    private void initComponents() {
        ...
        checkbox = new ComboCheckBox(new ArrayList<String>(Arrays.asList(nhaCungCapBUS.getHeaders())));
...
//        cb.setLocation(150, 115);
        ...
        nhaCungCap_Table = new QLNhaCungCapTable().getTable();
        nhaCungCap_Table.setBounds(42, 150, 860, 440);
        jPanel1.add(nhaCungCap_Table);
    ...
    private void searchtextfieldKeyReleased(java.awt.event.KeyEvent evt) {
        ...
        //ArrayList <QLNhaCungCapDTO> loaiSach = nhaCungCapBUS.search_all(search_str);

        /** Testing **/
        ArrayList <String> columns_checked = checkbox.getChecked();

        ArrayList <QLNhaCungCapDTO> nhaCungCap = nhaCungCapBUS.getArrNhaCungCap();
        ArrayList <QLNhaCungCapDTO> search_res = new ArrayList<>();
        ArrayList <QLNhaCungCapDTO> search_temp = new ArrayList<>();
        for (String e : columns_checked){
            search_temp = nhaCungCapBUS.search(e, search_str);

            ArrayList <String> pkey_1 = new ArrayList<>();
            ArrayList <String> pKey_2 = new ArrayList<>();

            for (QLNhaCungCapDTO ele : search_res){
                pkey_1.add(ele.getMaNCC());
            }

            for (QLNhaCungCapDTO ele : search_temp){
                pKey_2.add(ele.getMaNCC());
            }

            search_res = nhaCungCapBUS.getNhaCungCap_full(new DataProcessing().union_arr(pkey_1, pKey_2));
        }

        nhaCungCap = search_res;
        /** End Testing **/

        /* Print Ma Sach
        for (QLNhaCungCapDTO e : loaiSach){
            System.out.println(e.getMaSach());
        }*/

        jPanel1.remove(nhaCungCap_Table);
        jPanel1.repaint();
        jPanel1.revalidate();
        nhaCungCap_Table = new QLNhaCungCapTable().getTable(nhaCungCap);
        nhaCungCap_Table.setBounds(42, 150, 860, 440);
        jPanel1.add(nhaCungCap_Table);
    }

    public void paintTable(ArrayList <QLNhaCungCapDTO> nhaCungCap){
        jPanel1.remove(nhaCungCap_Table);
        jPanel1.repaint();
        jPanel1.revalidate();
        nhaCungCap_Table = new QLNhaCungCapTable().getTable(nhaCungCap);
    ...
    private void expand_btnMouseClicked(java.awt.event.MouseEvent evt){
        new QLNhaCungCapTable().expandMode();
    }
    ...
    private static JPanel nhaCungCap_Table;
